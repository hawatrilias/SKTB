...
// Ambil transaksi yang belum masuk ke tabel pemasukan
...
if ($result_transaksi && mysqli_num_rows($result_transaksi) > 0) {
    while ($row = mysqli_fetch_assoc($result_transaksi)) {
        // Escape data supaya aman
        $id_transaksi = mysqli_real_escape_string($conn, $row['id_transaksi']);
        $tanggal = mysqli_real_escape_string($conn, $row['tanggal']);
        $keterangan = mysqli_real_escape_string($conn, $row['nama_pelanggan'] . " - " . $row['produk']);
        $nominal = mysqli_real_escape_string($conn, $row['total']);
        $sumber = 'Penjualan';

        // Cek apakah transaksi sudah ada di tabel pemasukan
        $cek = "SELECT id_pemasukan FROM pemasukan WHERE id_transaksi='$id_transaksi'";
        $cek_result = mysqli_query($conn, $cek);

        if ($cek_result && mysqli_num_rows($cek_result) == 0) {
            // Masukkan data transaksi ke tabel pemasukan
            $insert = "INSERT INTO pemasukan (tanggal, keterangan, nominal, sumber, id_transaksi) 
                       VALUES ('$tanggal', '$keterangan', '$nominal', '$sumber', '$id_transaksi')";
            mysqli_query($conn, $insert);
        }
    }
}

// Ambil semua data pemasukan untuk ditampilkan di tabel
...
$sql = "SELECT * FROM pemasukan WHERE 1";
...
<head>
    ...
    <title>Pemasukan - Floristy Muse</title>
            ...
            <div class="table-container">
                <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:15px;">
                    ...
                    <div style="margin-left: 30px; display:flex; gap: 10px;">
                        ...
                        <a href="tambah_pemasukan.php" class="add-btn">Tambah Pemasukan</a>
                    </div>
                </div>
                <table>
                    <thead>
                        ...
                        <?php if (mysqli_num_rows($result) > 0): ?>
                            <?php while ($row = mysqli_fetch_assoc($result)): ?>
                                <tr>
                                    ...
                                    <td><?= $row['sumber'] ?? '-'; ?></td>
                                    <td class="action-btn">
                                        <a href="edit_pemasukan.php?id=<?= $row['id_pemasukan']; ?>" class="edit"><i
                                                class="fas fa-edit"></i></a>
                                        <a href="hapus_pemasukan.php?id=<?= $row['id_pemasukan']; ?>" class="delete"
